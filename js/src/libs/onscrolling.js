(function (global, factory) {
    ...
    function handleScroll() {
    	...
    	if (scrollY !== scrollYCached) {
            changeDirectionY = scrollY > scrollYCached ? 'down' : 'up';
            callbackQueue.y.forEach(triggerCallback.y);
            scrollYCached = scrollY;
            isScrollChanged = true;
        }
    	if (scrollX !== scrollXCached) {
            changeDirectionX = scrollX > scrollXCached ? 'right' : 'left';
    ...
    // Utilities
    // ---------
    function triggerCallback(callback, scroll, arg) {
        callback(scroll, arg);
    }
    triggerCallback.y = function(callback) {
        triggerCallback(callback, scrollY, changeDirectionY);
    };
    triggerCallback.x = function(callback) {
        triggerCallback(callback, scrollX, changeDirectionX);
    };
